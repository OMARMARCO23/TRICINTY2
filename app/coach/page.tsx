...
export default function AiCoach() {
  ...
  const send = async (e: React.FormEvent) => {
    ...
    const avgDailyUsage = currentUsage > 0 && daysSoFar > 0 ? currentUsage / daysSoFar : 0;
    ...
    try {
      ...
      const aiMsg = { role: "model" as const, parts: [{ text: data.message }] };
      setChatHistory((prev) => [...prev, aiMsg]);
    } catch (err: any) {
      const fallback = { role: "model" as const, parts: [{ text: err?.message || "Sorry, I couldn't connect. Try again in a moment." }] };
    ...
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      ...
      <form onSubmit={send} className="p-2 bg-base-100 flex gap-2">
        <input
          ...
          placeholder="Ask me how to reduce your bill..."
          value={input}
